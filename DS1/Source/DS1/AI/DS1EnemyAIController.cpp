...
void ADS1EnemyAIController::SetTarget(AActor* NavTarget) const
{
	if (IsValid(Blackboard)) 
	{
		Blackboard->SetValueAsObject(FName("Target"), NavTarget);
	}
	...
	if (IsValid(ControlledEnemy)) 
	{
		...
		{
			RotationComponent->SetTargetActor(NavTarget);
		}
	}
