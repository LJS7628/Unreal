...
float ADS1Character::TakeDamage(float Damage, const FDamageEvent& DamageEvent, AController* EventInstigator, AActor* DamageCauser)
{
	...
	// 적과 대치중인 방향인지?
	bFacingEnemy = UKismetMathLibrary::InRange_FloatFloat(GetDotProductTo(EventInstigator->GetPawn()),-0.1f,1.0f);
	...
	if (ParriedAttackSucceed()) 
	{
		if (IDS1CombatInterface* CombatInterface = Cast<IDS1CombatInterface>(EventInstigator->GetPawn())) 
		{
			...
			ADS1Weapon* MainWapon = CombatComponent->GetMainWeapon();
			if (IsValid(MainWapon))
			{
				FVector Location = MainWapon->GetActorLocation();
...
void ADS1Character::ShieldBlockingEffect(const FVector& Location)
{
	if (BlockingSound)
	{
